...
%     end
    ...
    %%manage SRM empty
    if ~isempty(SRMburst) && ~isempty(SRMspike) 
        mat_statistic_burst = [SRMburst(:,1),SRMspike(:,5) ,SRMspike(:,6), SRMburst(:,3),SRMburst(:,4),...
            SRMburst(:,5),SRMburst(:,6),SRMburst(:,7),SRMburst(:,8),SRMburst(:,9),SRMburst(:,10)];
    else
        mat_statistic_burst = zeros(1,11);
    end

     % Mean Stat Report MAT and TXT - Burst
    cd (end_folder1)
    nome=strcat('MAT_SMR', filename(16:end));
    save(nome, 'mat_statistic_burst')
    tableData = array2table(mat_statistic_burst, 'VariableNames', {'Channel', 'PercrndSpk', 'pfb','mbr','TotnBurst',...
        'meanSpkxBurst','STDspkxBurst','meanBD','stdBD','meanIBI','stdIBI'});
    [~, tmp, ~] = fileparts(nome);
    cleanedFilename = tmp;
    writetable(tableData, strcat(cleanedFilename,'.csv'));
    clear nome
