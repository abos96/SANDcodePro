function [S,C,L] = small_world_ness2(A,LR,CR,FLAG)
...
L = mean(D(D~=Inf));  % mean shortest path-length: including self-loops

% calculate required form of C
switch FLAG
    case 1
        c = clustering_coef_bd(A);  % vector of each node's C_ws
        C = mean(c);  % mean C
    case 2
        C = clusttriang(A);
end
