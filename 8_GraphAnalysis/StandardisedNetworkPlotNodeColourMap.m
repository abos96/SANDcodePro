function [figureHandle, cb] = StandardisedNetworkPlotNodeColourMap(adjM, coords, edge_thresh, z,...
    zname, z2, z2name, plotType,figFolder,minNodeSize)
...
if strcmp(plotType,'MEA')
    ...
    for i = 1:length(adjM)
        if z(i)>0
            nodeSize = max(minNodeSize, z(i)/nodeScaleF);
        ...
        if includeChannelNumberInPlots 
            pos = [xc(i)  yc(i)];
