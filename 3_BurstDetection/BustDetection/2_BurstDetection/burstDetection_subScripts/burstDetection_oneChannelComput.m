...
if str2double(addISImaxTh(1,4)) && str2double(addISImaxTh(1,2)) < param.maxISImaxTh
    % if addISImaxTh(1,4)
    if param.ISImaxTh < str2double(addISImaxTh(1,2))
        ISImaxsample = round(param.ISImaxTh/1000*param.sf);          % threshold for ISImax [sample]
        addWinsample = round(str2double(addISImaxTh(1,2))/1000*param.sf);
        amplFlag = 1;
    else
        ISImaxsample = round(str2double(addISImaxTh(1,2))/1000*param.sf);          % threshold for ISImax [sample]
        amplFlag = 0;
    end
else
    ISImaxsample = round(param.ISImaxTh/1000*param.sf);          % threshold for ISImax [sample]
